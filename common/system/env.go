...
func GetEnvValue(key string, dtype string, defaultValue any) any {
	...
	if useDefault {
		...
		return ConvertStringToAnyDataType(dtype, value)
	}
}

func ConvertStringToAnyDataType(dtype string, value string) any {
	switch dtype {
	case "string":
...
func ConvertEnvDurationStringToTimeDuration(envDurStr string, defaultDurStr string) time.Duration {
	durX := GetEnvValue(envDurStr, "string", defaultDurStr).(string)
